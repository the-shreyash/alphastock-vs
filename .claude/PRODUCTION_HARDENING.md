...
| Data integrity | 5.5 | 5.5 | **8.5** | **Destructive restore drilled**: 3 collections dropped → full recovery, 16 matched; backup `docker` transport verified. **PH3.8 audited the analytics/data model end to end** — every metric classified in code, one documented timezone strategy, ten defects fixed including two P0s (the end-of-day job **crashed on every run**, and the P&L it would have sent was the **platform-wide sum broadcast to every user as their own** — a cross-tenant disclosure). **PH3.9 (2026-08-16) then removed every fabricated metric: 17 → 0** (4 REAL / 32 DERIVED / 0 MOCK / 17 UNAVAILABLE), with the zero asserted by test. Six became real; eleven report an explicit UNAVAILABLE with the production data that would answer them. Revenue is gated on whether a **payment integration exists**, not on whether the collection is empty — so the first stray document cannot flip it back to "available". D-4 fixed: the refund endpoint no longer audits refunds it never performed. The fabricated backtest — always a 50–80% win rate by construction, rendered in the same cards as a real run — is deleted. −1.5: no off-host copy, no PITR, and the platform still records no payment, subscription, activity-event or token-accounting data, so seventeen legitimate metrics remain unanswerable (each named, with its required source) |
