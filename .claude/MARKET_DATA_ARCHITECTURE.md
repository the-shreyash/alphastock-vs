...
**D5.12 — health-recovery re-verification (ADR-052).** An audit sprint that added **no mechanism and changed no production file**, because the deadlock it was asked to close was already closed.
  ...
  * **One finding, and it is not a deadlock (LIM-D5.12-1).** `status()` resolves through the same path a request does, so during a sustained total outage of the only provider serving a capability the consumer-facing feed state flips to `available` at each cool-down expiry — before anything has answered — and back to `unavailable` when the trial fails. Nothing is fabricated and the blink is bounded by the ladder. Making `status()` ignore probes would remove it at the price of a consumer surface that disagrees with resolution, so it is pinned as it stands and left to the sprint that owns the consumer contract — the one that also owes LIM-D5.5-2.
