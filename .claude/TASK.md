...
- [~] PH3.2 Mock Data Eradication (ADR-021) — **AUDIT HALF COMPLETE (2026-08-16); removal half NOT_STARTED** — High — *Audit delivered under the sprint label "PH3.8 — Analytics & Data Integrity"; report: `docs/architecture/ANALYTICS.md`.* **Numbering note:** the sprint brief called this PH3.8 and its follow-on PH3.9; this tracker's PH3.8 is *Accessibility & Responsive Audit*, untouched. Read "PH3.9" in that report as **this line item's remaining half**. **The audit deliberately removed nothing.** Seventeen mock metrics stay in production with unchanged values, because ripping a chart out of a dashboard without its replacement is not an improvement; what changed is that every one now declares itself in its API response (`provenance` / `status` / `note` / `mock_metrics`) and renders behind a visible "Simulated" marker, with tests holding the flag in place until the removal sprint lands. **The inventory is code** — `backend/analytics/registry.py` classifies every metric REAL / DERIVED / MOCK / UNAVAILABLE with source, calculation, window, consumer and, for mocks, the production source that would replace it; `tests/test_analytics.py` asserts every endpoint it names exists on the live route table and every mock carries a replacement plan, so it cannot drift. **Totals: 4 REAL, 26 DERIVED, 17 MOCK, 5 UNAVAILABLE.** **The structural finding is that `db.payments` has no writer anywhere in the codebase** — the platform has no payment integration; three admin endpoints read the collection and startup indexes it, and nothing has ever written to it. So every revenue figure is fabricated, and `revenue_today` reads ₹0 only *because the collection is empty*: it is `count(all payments) × ₹499`, so the first record to land reports ₹499 of "today's revenue" whatever it was for. MRR/ARR infer revenue from role counts × hardcoded prices, and roles are granted through `grant-plan` with no payment, so comped and internal accounts count as paying. **Ten defects found and fixed, each reproduced first** — the roadmap entry carries the full list; the two P0s are that **`eod_report_job` crashed on every run** (`None.startswith` on an open trade's explicit `exit_time: None`, swallowed by a broad `except`, so no EOD report was ever written and no user ever notified) and that **the P&L it would have sent was the platform-wide sum, broadcast to every user as their own** — a wrong personal number and a cross-tenant disclosure. **New package `backend/analytics/`** (periods / contract / registry / queries / quality) — it computes no business metrics of its own; `portfolio_engine` and `trading_engine` remain the single source of truth for the math, and what moved is the *scoping*, which is what had drifted. **Three indexes added**, `portfolio_snapshots {user_id,date}` unique among them — that collection had no index of any kind since Sprint 8. **Tests: +122 backend, +11 frontend; backend 2,303 → 2,425 green, frontend 364 → 375 green**, every pre-existing test unchanged. **Remaining half (the removal sprint):** the 17-item priority-ordered specification is `docs/architecture/ANALYTICS.md` §11 and `analytics.registry.ph39_inventory()`. Four of its items (admin API health, AI provider latency, Redis/scheduler status, AI request counts) **need no new data** — PH3.7 already ships real probes and metrics; they are wiring, and they are the highest value-per-hour work in the list.
