...
- [ ] D5 / Phase 5 — Hardening: latency scoring, flap suppression, probation windows, multi-connection sharding, chaos tests — **D5.1 (reconnect flap suppression, closes DB-5) COMPLETE (2026-08-25, ADR-041)**; **D5.2 (provider probation — READY vs STABLE, closes LIM-D5.1-3) COMPLETE (2026-08-27, ADR-042)**; **D5.3 (provider stability decay & stale-feed demotion, closes LIM-D5.2-3) COMPLETE (2026-08-27, ADR-043)**; **D5.4 (provider delivery-latency scoring, closes LIM-D5.3-3) COMPLETE (2026-08-27, ADR-044)**; **D5.5 (entitlement-failure classification & safe recovery, closes the D4.11 code-806 approximation) COMPLETE (2026-08-27, ADR-045)**; **D5.6 (generalized provider re-probe & safe recovery, closes LIM-D5.5-3) COMPLETE (2026-08-29, ADR-046)**; **D5.7 (provider health recovery — the failure cool-down, closes the ADR-029 deadlock and LIM-D5.6-1) COMPLETE (2026-08-29, ADR-047)**; **D5.8 (DB-1 — distributed provider/broker health and recovery state, closes LIM-D5.7-2) COMPLETE (2026-08-29, ADR-048)**; **D5.9 (p95 delivery latency and latency inside `health()`, closes LIM-D5.4-3) COMPLETE (2026-08-29, ADR-049)**; **D5.10 (instrument sharding across several broker connections, closes the trim-instead-of-shard limitation recorded in D4.6/D4.7/D4.9/D4.10/D4.11 for four of the five brokers) COMPLETE (2026-08-30, ADR-050)**; D5.11+ (exchange-timestamp latency, chaos tests) not started
...
- Kite's 3,000-instrument-per-connection cap is **sharded as of D5.10** (ADR-050). A retail portfolio is nowhere near it, so the path is exercised only by tests.
...
- **LIM-D5.4-3 — p50 only; no p95, and no latency in `health()`.** ✅ **CLOSED (D5.9, 2026-08-29, ADR-049).** Both halves are implemented. The p95 is the *same* interval series read over a wider window — the deque widened from 9 to `LATENCY_TAIL_WINDOW_SAMPLES = 20`, the median still reads its newest 9, so the D5.4 definition and every ranking property are unchanged. 20 is **derived**: `ceil(0.95 * N)` equals `N` for every `N` below 20, so a p95 over nine samples is literally the maximum — which is the reasoning below, carried one step further than D5.4 carried it. `health()` now returns a `LatencyProfile` (`established / p50_seconds / p95_seconds / samples`) **derived on every read** rather than accumulated, which honours the original objection: it is not a counter, cannot drift, and cannot cache a cadence the feed has lost. p95 is reported and is deliberately not a ranking term. Original text preserved: §7 asks for rolling p50/p95 and the adapter table named "measured latency" on `health()`. p95 of nine samples is "the largest one", which is an outlier detector rather than a score, and a sample large enough for a real p95 is a warm-up long enough to be a liability. `ProviderHealth` was left alone because health is counter-based evidence from past *calls* and a pushed feed makes none — folding a push-derived statistic into it would be the transport/evidence unification ADR-043 refused, one layer along. The adapter contract table has been corrected to say so rather than continuing to promise it.
...
- **LIM-D5.5-3 — a refused feed never retries, and nothing re-probes it.** ✅ **CLOSED (D5.6, 2026-08-29, ADR-046).** A refused feed is now recorded as a `REPROBE` recovery candidate and re-attached on its own paced ladder (300s → 3600s), per (user, broker, channel), blocked while the session is dead or the channel is already attached, and discharged by market data arriving. Original text preserved: Intended, and also a one-way door until a lifecycle event: an entitlement granted *while connected* does not bring the feed back until the user reconnects the broker or the process restores sessions. A generalized re-probe is Phase 5 work ADR-029 already owes for demoted providers; this is its second caller.
...
## D5.6 — Generalized Provider Re-Probe & Safe Recovery (2026-08-29) — COMPLETE, uncommitted

**ADR-046.** Closes LIM-D5.5-3 and answers the re-probe ADR-029 deferred to Phase 5 in D2.

**The audit's finding is that the deadlock is not general, and that listing the ones that are is more than half the sprint.** Seven of the ten states the brief names already recover correctly and were pinned rather than rebuilt: a transient disconnect through D5.1's ladder, a stale open feed on the next accepted tick through D5.3, a link-down demotion through the readiness gate on the new link, probation and unestablished latency (which cannot deadlock, because a pushed feed accumulates evidence without being selected — ADR-044's property), auth expiry (which correctly requires a new session), and both lifecycle re-attach paths. **Three genuinely deadlock:** the entitlement-refused feed (LIM-D5.5-3, this sprint's caller); a provider whose health reached DOWN (ADR-029 — excluded from candidates, never called, never able to improve, and for the polled baseline that is a total outage that never self-heals); and a stream whose channel or transport left the configuration. Only the first is fixed, and the other two are recorded as LIM-D5.6-1 and LIM-D5.6-2 with the reason.

**Recovery is classified, never generic.** Five `RecoveryClass` members, and only one is retried: `TRANSPORT` and `EVIDENCE` are **refused registration outright** because both already heal themselves and a candidate for a self-healing condition is a second mechanism racing the first; `REPROBE` is paced; `SESSION` is recorded and never retried; `CONFIGURATION` needs a deployment change. Collapsing them is the defect D5.5 found one layer down.

**The probe is one ordinary attach.** No control-plane probe, no `check_entitlement()` on the adapter contract, no new adapter method, no new capability. A re-probe calls `start_stream` scoped to the one withdrawn channel and reads its outcome off the callbacks that already exist. A control-plane "yes" would widen the contract for every adapter *and* prove the wrong thing — MARKET_DATA_ARCHITECTURE.md has one definition of a usable feed and it is a valid canonical tick on the current link.

**Recovery creates no eligibility.** `attach_market_feed` builds a *new* `StreamingTickProvider`, so a recovered feed re-earns READY, serves a full probation window and inherits neither readiness, nor stability, nor latency evidence. The property is asserted rather than the mechanism, so a future implementation that reuses the instance has to keep it true.

**The re-probe ladder is its own.** 300s → 3600s, doubling, jittered. Pinned by a test asserting the *slowest* reconnect (60s) is still faster than the *fastest* re-probe, so the two schedules cannot collapse into one. The base delay is honestly a new number; the equal-jitter arithmetic is D5.1's, reused because it is not connection semantics.

**A dead session is excluded twice, by guards that catch different facts** — classification for a token that expired, a session predicate at attempt time for a session that went away *afterwards*. **Only a lifecycle event resets the ladder** (`complete_auth`, `disconnect`); an apparent success does not, because a broker that accepts a socket and then refuses would otherwise reset the pacing every cycle. **The candidate is discharged by market data, not by a socket.**

**Files.** New: `backend/services/brokers/recovery.py` (the register, the five classes, the seven-member outcome vocabulary, the service and the bounded sweeper — imports nothing from `services.` except `reliability`, names no broker anywhere including comments), `backend/tests/test_provider_recovery.py` (51 tests). Changed: `backend/services/broker_engine.py` only — two withdrawal recordings, one evidence discharge, two lifecycle clears, three injected predicates, a `channels=` filter on `start_stream` whose default reproduces the previous behaviour byte for byte, a gate so an order-channel re-probe cannot replace a live market feed, and sweeper start/stop. **No market-engine module, no `MarketTick`, no `StreamEventKind`, no broker adapter, no provider contract, no capability, no gateway, no registry, no REST route and no frontend file was touched.**

**Tests.** All 24 areas of the brief: the refusal still terminal and now recorded; a re-probe while still refused producing no serving provider; the ladder climbing 300 → 600 → 1200 and capping, with 100 undue asks producing zero attaches; entitlement restoration recreating the stream through the engine's own path; the recovered feed earning READY and probation and inheriting nothing (parametrised over readiness, stability and latency); Yahoo present and the status `available` at every step of a full cycle; auth expiry recorded as `SESSION`, downgrading an entitlement candidate on every channel, and never queued; a due candidate whose session vanished never attempted and never charged; a new session clearing both the withdrawal and the ladder; a transport class refused registration and an ordinary link loss creating no candidate; stale-feed natural recovery pinned *with* the assertion that no candidate was created; two users on one broker, two brokers of one user and two channels of one account all independent; the guest and baseline byte-identical; a live channel never torn down by a sweep; the consumer status asserted by exact key set and by leak sweep; comment-inclusive broker-name sweep, executable-code sweep and identity-branch sweep; an AST import sweep proving the module reaches nothing beyond its layer; a fictional broker using the contract; and the Market Engine importing none of it.

**Falsification.** **Twenty-eight mutations, twenty-eight red** — all sixteen the brief names plus twelve more: the attempt charged after the attach rather than before, the discharge clearing the ladder, the session guard removed, the already-attached guard removed, transport blips made registrable, the ladder uncapped, a re-probe reattaching every channel, an order-channel re-probe replacing the market feed, link-up discharging instead of evidence, the withdrawal never recorded at all, forget-on-reconnect removed, and the auth expiry recorded as re-probeable at the engine as well as at the class. Every one was re-checked to fail on a targeted assertion rather than on an import or collection error.

**Regression.** Measured immediately before editing: 3679 passed, 15 failed, 4 xfailed, 95 deselected. After: **3730 passed (3679 + 51 new), 15 failed — the identical documented `test_entrypoint_log_level.py` Docker baseline — 4 xfailed, 95 deselected.** No new failures. The D4/D5 suites also pass under the default random ordering (809 tests), which is what proves the new module-level register does not leak between tests. flake8 clean (both gates) on every new and changed file; the two findings on `broker_engine.py` are pre-existing at HEAD and on lines this sprint did not touch. isort clean on both new files. `black --check` reports the same advisory reformats it reports at HEAD on the files this sprint touched, and nothing was mass-formatted.

**Security.** The full cycle — refusal, record, paced re-probe, attach, second refusal, expiry, reclassification — driven at DEBUG through the real logging stack with a live-looking JWT access token, a refresh token, a feed token, an API key and an API secret. **None of the five appears in any log line, nor does a 12-character prefix of any of them, nor `Bearer `, nor an `authorization` header name, nor a `?token=` query string.** Recovery log lines carry a broker name, a channel name, a user id and two counters. The diagnostics row is asserted by exact key set and swept for credential vocabulary. The consumer `provider.status` is unchanged in shape and carries no recovery vocabulary, no broker identity and no attempt count.

**LIVE VALIDATION: NOT PERFORMED.** No interactive broker session exists in this environment. The outstanding smoke test is in ADR-046.

**Deliberately NOT done in D5.6** — no DB-1 health relocation, no p95 or exchange-timestamp latency, no instrument sharding, no chaos suite, no Groww, no INDmoney, no frontend explanation of entitlement failure (LIM-D5.5-2 stands), no trading changes, no new broker adapters, no D5.7; the ADR-029 **health** deadlock was audited, classified and deliberately left without a caller (LIM-D5.6-1); nothing committed and nothing pushed.

**Limitations carried out of D5.6.**
- **LIM-D5.6-1 — the ADR-029 health deadlock is still open.** ✅ **CLOSED (D5.7, 2026-08-29, ADR-047)** — and by implementing a clause the authoritative document had always carried rather than by inventing a mechanism: step 2 of the Resolution procedure reads "`down` **or inside a failure cool-down**", and only the first half existed. Original text preserved: A provider that reaches DOWN is excluded from `candidates_for`, is therefore never called, and can therefore never improve; for the polled baseline that is a feed outage surviving until a process restart or an external `record_success`. D5.6 classified it and stopped: re-probing a DOWN provider means *calling* it, which is the Market Engine's business, and the Market Engine may not import the broker layer — so it needs its own caller, its own probe symbol, and a decision about whether an excluded provider is re-admitted to the tail of the chain or probed out of band. That is a resolution-path change with real regression surface. `test_a_demoted_provider_has_no_self_recovery_path_in_d2` still passes, unchanged.
- **LIM-D5.6-2 — `RecoveryClass.CONFIGURATION` is classified and never recorded.** The two transport paths that are terminal for configuration reasons return without a callback, and adding one would widen `BrokerStream` for a condition that is correctly unrecoverable anyway. The class exists so the register refuses to re-probe one if a caller ever records one, which is pinned.
- **LIM-D5.6-3 — a re-probe the broker refuses again costs one connection.** The mechanism working as designed; there is no cheaper way to ask. Bounded by the ladder rather than eliminated: an account refused for a day makes roughly a dozen attempts, against a reconnect ladder whose *ceiling* is one per minute.
- **LIM-D5.6-4 — the register is process-local**, like `BrokerHealth` (DB-1). One register per worker, which is correct (the worker holding the session is the one that re-probes), but a restart loses every ladder and every candidate, and the first sweep afterwards starts at the base delay. Acceptable while a restart re-attaches every session anyway.
- **LIM-D5.6-5 — no live validation.** See ADR-046 for the outstanding smoke test.

## D3 — Debts carried into D4

- **DB-1 — Broker health is process-local.** ~~`BrokerHealth` lives on the registry's adapter instance, so a multi-worker deployment has one health view per worker and the Admin Portal sees whichever worker answers.~~ **CLOSED in D5.8 (ADR-048).** Broker health, polled-provider health and D5.7's failure cool-down are now one shared Redis record with atomic transitions and an atomic recovery-trial claim; `broker_gateway.health_shared()` is the operator read. Live-socket state (readiness, probation, freshness, latency) and D5.6's REPROBE register stay process-local by argument — see LIM-D5.8-3.
...
## D5.7 — Provider Health Recovery: the ADR-029 Failure Cool-Down (2026-08-29) — COMPLETE, uncommitted

**ADR-047.** Closes the health half of the ADR-029 deadlock and LIM-D5.6-1.

**The audit's central finding is that the mechanism was already specified, and half of it had been built.** MARKET_DATA_ARCHITECTURE.md's Resolution procedure, step 2, has read "filter out candidates whose health state is `down` **or that are inside a failure cool-down**" since before D1. D1 implemented the first half as an unconditional filter and the second half not at all — and a cool-down with no expiry is a permanent exclusion, which is the whole of the bug. Failover rule 4 and the failover diagram's closing line ("Retry loop continues; first provider to recover restores the feed") say the same thing twice more. So D5.7 adds no policy; it finishes a sentence.

**The audit's second finding narrows the deadlock, and the narrowing is what made it fixable.** It is specific to a provider whose only evidence comes from *being called*. A pushed feed records a success from `_ingest_ticks` whether or not it is selected — ADR-044's property, re-verified here. A DEGRADED provider is not excluded at all: it stays a candidate, stays in the chain, and recovers the moment the chain reaches it; it is *unreached*, not *unreachable*. Only a **polled** provider at DOWN closes the cycle, and the only polled provider the platform ships is the permanent baseline — so the untreated case was never a corner case, it was a total feed outage that survived until a process restart. **A correction is recorded rather than smoothed over:** ADR-029's prose ran DEGRADED and DOWN together, and ADR-046 cited `test_a_demoted_provider_has_no_self_recovery_path_in_d2` as the pin for LIM-D5.6-1. That test is about DEGRADED. It remains true, it is unchanged, and it never pinned the deadlock this sprint closed.

**The chosen mechanism: request-triggered re-admission at the tail, charged by evidence.** `resolve_feed` asks a new `ProviderHealthRecovery` register whether any DOWN-but-otherwise-eligible provider's cool-down has run, and appends the ones that have. No task, no sweeper, no timer, no `asyncio` — pinned by a test, and answering brief rule 7 the way the rule asks: the resolution path already runs several times a minute and had simply stopped including the provider in the ask. `HEALTH_RANK` gains `DOWN: 2`, the worst band, and health is the first element of the selection key — so "a re-probed provider never outranks a healthy or probationary one" is true by the position of an element rather than by a branch. The gateway reaches the tail exactly when it would otherwise have returned nothing, so the trial costs nothing in the normal case and is strictly better in the outage case.

**Re-admission is not recovery.** Health is untouched by being offered a trial and stays DOWN until a real `record_success` from a real call; resolving a hundred times moves no counter. `record_success` clears the cool-down gated on the provider's *resulting state*, so an empty success — which does not reset the failure streak — clears neither. The ladder is charged from `record_failure` when the provider is still DOWN afterwards, which makes the pacing exact: at most one call per cool-down, and a provider offered but never reached keeps its trial.

**Why it does not compete with D5.6's REPROBE, proved from the architecture rather than assumed.** D5.6's unit is `(user, broker, channel)` and its probe is one ordinary *attach of a broker stream*; a DOWN provider may be the polled baseline, which has no broker, no channel and no attach. Its register lives in `services/brokers/`, which the Market Engine may not import at all. And REPROBE exists for conditions nothing in this process observes changing, whereas a DOWN provider's condition is this process's own calls failing — which it observes directly. Two mechanisms, no shared code, no shared constant, and the boundary is now swept in **both** directions.

**Pacing.** 60s base, doubling, 240s ceiling, no jitter. The base is honestly a new number and the reasons for not borrowing 30s/120s/300s are recorded in the module. The **ceiling is derived**: 240 < 300 keeps the slowest health cool-down faster than the fastest D5.6 re-probe, which is the right way round because a DOWN provider is machine-timescale and an unresolved entitlement is human-timescale. Jitter is omitted deliberately — it decorrelates a herd released by a shared schedule, and there is no schedule.

**Files.** New: `backend/services/market_engine/providers/health_recovery.py`, `backend/tests/test_provider_health_recovery.py`. Changed: `providers/registry.py` (`_eligible_for` factored out, `down_candidates_for` added, the DOWN filter itself untouched), `providers/__init__.py` (exports), `source_manager.py` (the `DOWN` rank row, the probe step in `resolve_feed`, the two bookkeeping hooks, `forget_health_recovery`, `reset`, `diagnostics`), `market_engine/gateway.py` (one `forget` call on unregister), `tests/test_provider_recovery.py` (D5.6's boundary sweep, below). **No broker adapter, no transport, no codec, no `MarketTick`, no capability, no event kind, no API, no route and no frontend file was touched.** The audit found no broker-contract insufficiency, so BROKER_INTEGRATION.md needed no change.

**One existing test was rewritten, and it is reported rather than quiet.** D5.6's `test_the_market_engine_imports_no_recovery_implementation` asserted `"recovery" not in source` over four market-engine modules. The property it defends is that the Market Engine never reaches for the *broker layer's* re-probe machinery; the bare word was a proxy that worked only while no market-engine module had a recovery concept of its own. It now lists D5.6's actual symbols, the structural ban on importing `services.brokers` is untouched, `health_recovery.py` was added to the swept set, and a new test asserts the same boundary in the other direction — `services/brokers/recovery.py` may not name provider health.

**Tests.** 40, all through the real seams where one exists: the real `MarketGateway` request path drives every health transition; the real `attach_market_feed` / `publish_market_ticks` seam builds the feeds; the real `BrokerEngine._on_stream_not_entitled` and `_on_stream_expired` callbacks drive the two withdrawal paths. Covered: the DOWN transition counted by the gateway; exclusion inside the cool-down and the zero further calls it buys; re-admission once it runs; re-admission leaving health byte-identical over a hundred resolutions; successful recovery; failed recovery re-excluding and doubling; the ladder charged by evidence and never by the offer; three full DOWN→trial→failed trial→second trial→recovery cycles; the ceiling as a ceiling and not a give-up; never outranking a healthy provider and never outranking a probationary one; recovery restoring health and *not* stability; readiness and per-symbol coverage still refusing a re-admitted feed; stale evidence (empty success) restoring nothing, through both the seam and the gateway; cool-downs unshared between providers and between users, including two feeds deliberately given the same name under different owners; one user's DOWN feed invisible to another; Yahoo registered, connected and serving at every stage of a cycle; the baseline's own total outage recovering end to end; entitlement refusal and session expiry unregistering rather than demoting, so neither can reach this mechanism; unregistration dropping the cool-down; a reconnected feed inheriting nothing; all five shipped brokers plus fictional Nova through the real attach seam; a comment-inclusive broker-name sweep with the one pre-existing D1 sentence quoted and exempted explicitly; an identity-branch sweep; a no-timer sweep; the two mechanisms' constants; the consumer payload asserted by exact key set at every stage; recovery state present only on the admin surface; and a DEBUG pass through the real logging stack with three live-looking credentials.

**Falsification.** Seventeen source-level mutations, **seventeen red**, each on a targeted assertion: remove the DOWN exclusion; remove the recovery trigger; make recovery unconditional; bypass readiness/entitlement on the probe path; rank probation above health; rank a re-admitted provider alongside a healthy one; let an empty success clear the cool-down; share state between users; share state between providers; make recovery broker-specific; suppress the baseline while probing; never charge the ladder; demote instead of unregistering on an entitlement refusal; remove the ceiling; re-admit only once so repeated cycles deadlock again; charge the ladder on the offer; and make the cool-down expire only on an exact instant. One test was strengthened as a result: `test_repeated_down_recovery_down_cycles_never_deadlock` originally recovered on its first trial and so did not catch the arm-once mutation; it now takes a failed trial in every cycle.

**Regression.** Baseline immediately before the change: 3730 passed, 15 failed (the documented Docker `test_entrypoint_log_level.py` set), 4 xfailed. After: **3771 passed, the same 15 failed, the same 4 xfailed**, in both fixed and randomized order. No new xfail. Blocking flake8 (`E9,F63,F7,F82,F811,F632`) clean repo-wide; the two new files are clean against the full advisory config as well, which is the new-file ratchet.

**Security.** A DEBUG pass through the real logging stack with a live-looking JWT, an API key and a refresh token on a Nova feed: no credential and no provider error text reaches the recovery logs, `provider.status` is unchanged by exact key set at every stage of a cycle, cool-down state appears only on the admin `diagnostics()` surface where provider names already live, and the register is keyed by owner so no user's recovery state is visible in another user's resolution.

**LIVE VALIDATION: NOT PERFORMED.** No interactive broker session and no reachable provider outage exists in this environment. Outstanding smoke test: take a real provider to DOWN by making eight consecutive calls fail (block the upstream host, or revoke the key); confirm the tier falls and that the next twenty requests make **no** further calls to it; wait 60 seconds and confirm exactly **one** call is attempted, at the tail of the chain, with the healthy provider still answering first if one exists; let that call fail and confirm the next attempt is 120 seconds out, then 240, then 240 again; restore the upstream and confirm the next due trial restores the tier with no restart and no manual `record_success`; then repeat with the baseline as the *only* provider, and confirm the feed goes from `unavailable` back to `delayed` on its own.

**Deliberately NOT done in D5.7** — no p95 or exchange-timestamp latency, no DB-1 health relocation, no instrument sharding, no chaos suite, no Groww, no INDmoney, no frontend explanation (LIM-D5.5-2 stands), no change to `MarketTick`, the broker contract, the transport, the REST quote path or the frontend, no D5.8; nothing committed and nothing pushed.

**Limitations carried out of D5.7.**
- **LIM-D5.7-1 — recovery is opportunistic, not scheduled.** A re-admitted provider is *offered* a trial on every resolution once its cool-down has run, but the gateway stops at the first provider that answers, so a DOWN provider sitting behind a healthy one is only actually called when the healthy one fails. That is deliberate and forced by brief rule 10: anything that guaranteed the trial would mean either ranking a known-broken provider above a working one, or a second out-of-band call, which is the extra mechanism rule 7 forbids. The provider is no longer unreach*able*, which is what the deadlock was; it is unreached, which is the same status a DEGRADED provider has always had. The case that mattered — the baseline alone, down — recovers fully, because there is nothing above it to answer first.
- **LIM-D5.7-2 — the register is process-local**, like `BrokerHealth` and like D5.6's (DB-1, LIM-D5.6-4). One cool-down per worker, which is correct while health itself is per worker, but it means N workers can each spend one trial per cool-down against the same provider. Bounded and small; it becomes a real decision when DB-1 relocates health, at which point "does one worker's trial discharge another's?" has to be answered.
- **LIM-D5.7-3 — the base delay is a new number and is not derived.** The ceiling is (it is pinned below D5.6's base delay, with the reason); 60 seconds is a judgement about how long a user should sit on no data against how often a broken provider should be asked. It is exercised at its published value throughout and is one constant to change if operational evidence disagrees.
- **LIM-D5.7-4 — a re-admitted trial can convert an empty default into a raised exception** on the `ALL_PROVIDERS_DOWN` path, because a chain of one exists where none did. Consistent with the D2 contract every other caller already lives under, and named rather than discovered later.
- **LIM-D5.7-5 — no live validation.** See above.

---

### D5.8 — DB-1: Distributed Provider/Broker Health & Recovery State ✅ (2026-08-29)

**ADR-048.** Closes DB-1 and LIM-D5.7-2.

**The audit's central finding is that "is this in memory?" is the wrong question, and asking it would have made the system worse.** The right question is *is this state evidence about something every worker observes?* A broker's API is one remote system, so every worker's calls to it are evidence about one outage. A polled provider — the permanent baseline — is registered in every worker and called by every worker over HTTP. Both shared. A **streaming provider** is one live socket held by one worker: its health, readiness, probation window, freshness evidence and delivery-latency intervals are evidence about *that link*, and D5.3 already discards all of it on reconnect. Publishing it would let a dead socket's DOWN verdict be inherited by the fresh link a *different* worker opened — the opposite of what D5.5 and D5.6 established. **Not shared**, and stated on the model as `MarketDataProvider.health_is_shared` so it is one discriminator rather than a rule each call site remembers.

**D5.6's REPROBE register stays local, and that is a decision.** It records the withdrawals *this worker's own sockets* suffered, and a re-probe is one ordinary attach. Sharing it would let worker B attach a channel whose stream lives in worker A — two sockets for one `(user, broker, channel)`, which is a worse failure than the one DB-1 fixes. LIM-D5.6-4 stands, restated as a choice.

**The real DB-1 defect is not the cosmetic one.** The visible complaints — the Admin Portal reporting whichever worker answered, a provider needing `8 x N` failures to be excluded — are real. The one that matters is that **D5.7 promised "at most one trial per cool-down" and delivered it per worker**, so a provider that is genuinely down was retried `N` times per cool-down. That is LIM-D5.7-2, and it is what the atomicity work is for.

**The contract obstacle, and the shape of the answer.** `resolve_feed` is synchronous and is called from routes, diagnostics, the scanner and the gateway; DB-1 needs an *atomic* decision on that path, and atomic means Redis, and Redis means `await`. Rather than make a five-module contract awaitable for a question asked about a handful of DOWN providers, the awaitable work is lifted into `SourceManager.prepare()` — one call per gateway request — whose result travels as a `SharedResolution` into the unchanged synchronous `resolve_feed`, where `due_from(claims=...)` **filters and re-decides nothing**. A caller that does not prepare gets exactly the D5.7 behaviour. Pinned by an AST test that `resolve_feed` is not a coroutine.

**Atomicity is the whole mechanism.** Every mutation is one Lua script — one atomic transition, one round trip — because `GET` → modify → `SET` loses exactly the updates that matter: two workers recording the seventh and eighth consecutive failure in the same instant must produce a streak of 8, and two workers finding the same trial due must not both take it. The scripts read the clock with `redis.call('TIME')`, because monotonic clocks are not comparable *between* processes and per-worker wall clocks would make the ladder as skewed as the worst-set clock in the fleet.

**The trial claim leases the offer, never the ladder.** D5.7's rule is that the ladder is charged by evidence and never by the offer. So a claim takes a 30-second exclusive lease on the *right to offer* a provider and touches neither `attempts` nor the next-probe instant; only a failed probe climbs the ladder, and the same write releases the lease. Both bounds are semantic: longer than the platform's longest provider HTTP timeout (12s) so a second worker cannot take the trial mid-call, shorter than the 60s base cool-down so a worker that died holding a lease cannot park a due trial for a whole cool-down.

**Key model.** `sa:health:{provider|broker}:{owner|-}:{name}`, with the trial key *derived from* the health key so the two can never be scoped differently. `kind` because a broker and a provider can share a name; `owner` because two users can hold a feed from the same broker and one account's outage is not the other's. No token, key, session or URL is ever written to a key or a value. **TTL one hour**, bounded in both directions: it must outlive D5.7's 240s ceiling or a key expiry would erase a failure streak and re-admit a provider with no evidence, and it must be finite or a per-user feed for a closed account holds a key forever.

**Redis unavailable: bounded local fallback.** Chosen from what the deployment already guarantees — Redis is `critical=False` in readiness, `services/cache.py` degrades to a dict, and `infrastructure/redis_client.py` exists to make degradation a first-class path. *Fail closed* would turn a blip in a non-critical dependency into a total feed outage; *fail open* would discard the evidence this worker holds. Every store method returns `(ok, ...)` and the caller applies the mutation locally with the code that has always been there, so the platform reverts to exactly its pre-D5.8 per-worker behaviour for the duration.

**The Lua state machine is not trusted; it is pinned against the Python one.** The transitions exist twice — Python for the fallback, Lua for atomicity — so parametrised parity tests replay the same event sequences through `BrokerHealth`/`ProviderHealth` and through the store and assert identical snapshots. The Python version is the oracle. No policy changed: the same two thresholds, four states, auth-failure exclusion and empty-success rule.

**Broker health gained the read path DB-1 originally asked for.** `broker_gateway.health_shared()` and `diagnostics_shared()` adopt the shared record before rendering, so an operator gets the deployment's answer instead of one replica's. The synchronous `health()` is unchanged for callers that cannot await.

**Files.** New: `backend/infrastructure/health_state.py`, `backend/tests/test_distributed_health.py`. Changed: `providers/base.py` (`health_is_shared`, `apply_shared_health`), `providers/streaming.py` (the opt-out and its reasoning), `providers/health_recovery.py` (`ProbeClaims`, `claim_due`, the three `*_shared` methods, `due_from(claims=)`), `providers/__init__.py` (exports), `source_manager.py` (`SharedResolution`, `prepare`, `resolve_feed(shared=)`, `record_success_shared`, `record_failure_shared`, `forget_health_recovery_shared`), `market_engine/gateway.py` (the prelude and three awaited bookkeeping calls), `brokers/health.py` (`apply_shared`, the four module-level shared functions), `brokers/gateway.py` (three awaited calls, `health_shared`, `diagnostics_shared`). **No broker adapter, no transport, no codec, no `MarketTick`, no capability, no event kind, no API, no route and no frontend file was touched.**

**Tests.** 60 in the new file, of which 41 run only against a real Redis and are skipped without one — deliberately, because atomicity is a *server-side* guarantee and a double that answers the way the author expected is the failure mode PH2.12 named. A "worker" in these tests is an independent object graph (its own registry, `SourceManager`, cool-down register and provider instances) over one shared store, which is what a second uvicorn process is as far as this state is concerned. Covered: a failure by A seen by B; B unable to reset A's streak; a success by A recovering the provider for B; a provider driven DOWN by A excluded by B *at resolution*; health surviving a worker restart; two and four workers claiming one trial; the losing worker not offering the provider at resolution either; a failed trial advancing one ladder both workers observe; a successful trial clearing the cool-down everywhere; an unreached offer costing nothing; user, provider, broker and provider-vs-broker-namespace isolation; broker DOWN on the deployment's eight failures; the admin read path; auth failure still outside the state machine; empty success still not clearing the streak; two, and eight, simultaneous failures; a failure racing a success; a claim racing a failure; a claim on an expired key arming rather than granting; eleven parity sequences against the Python oracle; the ladder rung-for-rung; four Redis-unavailable behaviours including that a single failure stays a single failure; the streaming opt-out asserted as behaviour on every write path; the key model; both TTL bounds and both lease bounds; a DEBUG credential sweep over the keyspace, the values and the logs with a password-protected Redis; cross-user invisibility at resolution; a broker-name sweep; an AST test that no second Redis client exists; a structural ban on read-modify-write; the AST test that resolution stayed synchronous; and three cost tests.

**Falsification.** Twenty-five source-level mutations, **twenty-five red**. Three started green and two of those were malformed rather than survived: replacing the per-field `HGET` with an in-script snapshot is still atomic (redone as a real Python read-modify-write → red, killing `test_many_simultaneous_failures_reach_down_exactly_at_the_threshold` and the structural ban), and a local `reset_health()` immediately before `apply_shared_health` is overwritten by the next line (redone as a mirror that zeroes the adopted streak → red). **One was a genuine gap**: "Redis failure marks every provider DOWN" survived, because the fail-open test drove eight real failures and the fail-closed test drove none, so nothing asserted that degradation stays *proportional* to evidence. `test_a_single_failure_with_redis_down_is_still_a_single_failure` was added and the mutation is red.

**Regression.** Baseline immediately before the change: **3766 passed, 4 xfailed, 95 deselected** (`tests/` minus the documented Docker file `test_entrypoint_log_level.py`, whose 15 failures are unchanged and unexamined). After: **3826 passed, the same 4 xfailed**, under the suite's randomized ordering. Without a Redis reachable the new file is 19 passed / 41 skipped and the D4/D5 suites are 744 passed — the CI path is green either way. No new xfail. flake8 clean on every changed and new file under the repository's full config.

**Performance.** One Redis operation per resolution with nothing down (one batched read of the whole candidate set), plus one per *currently DOWN* provider; one per health mutation; one per broker diagnostics render regardless of broker count. Measured against a local Redis: `record` p50 0.295 ms / p95 0.357 ms, `read_many(1)` p50 0.256 ms, `read_many(5)` p50 0.244 ms, `claim` p50 0.232 ms. No loop, no retry beyond the client's own budget, and a deployment with no Redis pays nothing. The claim is one script per key rather than one multi-key script, deliberately: a multi-key script is a Redis Cluster error when the keys hash to different slots, and `infrastructure/redis_client.py` names Cluster as the migration its seam exists for.

**Security.** DEBUG through the real logging stack against a **password-protected** Redis, so the connection credentials were present and findable: no password, token or session material reaches a key, a value or a log line; `sanitized_url` redaction is unchanged; the admin diagnostics surface carries no connection detail; and cross-user invisibility is asserted at resolution rather than only at the key.

**LIVE VALIDATION: NOT PERFORMED.** No multi-worker production-like deployment and no interactive broker session exists in this environment; the multi-worker tests run against a local Redis container, which is not the same claim. Outstanding smoke test: run two uvicorn workers against one Redis; (1) drive a provider to eight consecutive failures through worker A and confirm worker B's next resolution excludes it without ever having called it; (2) confirm `GET /admin` broker health reports the same state from both workers; (3) let the cool-down run and hit both workers simultaneously, confirming exactly **one** upstream call is made; (4) restart worker A and confirm the provider is still DOWN — the logical health state survives the process; (5) stop Redis and confirm both workers keep serving, that a single provider failure remains a single failure, and that recovery pacing reverts to per-worker; then restart Redis and confirm the shared record is re-established by the next mutation rather than by a push of local state.

**Deliberately NOT done in D5.8** — no p95 or exchange-timestamp latency, no instrument sharding, no chaos suite, no generalized re-probe beyond what DB-1 required, no distribution of D5.6's REPROBE register (argued, not omitted), no Groww, no INDmoney, no frontend explanation (LIM-D5.5-2 stands), no change to `MarketTick`, the broker contract, the transport, the REST quote path or the frontend, no D5.9; nothing committed and nothing pushed.

**Limitations carried out of D5.8.**
- **LIM-D5.8-1 — the local mirror can be one mutation stale.** A worker that has neither recorded nor read since another worker's last write holds the previous value. Inherent to a mirror, and why the operator-facing broker read is `health_shared` and why the market resolution path always prepares. It matters only to a caller that reads a health counter without either.
- **LIM-D5.8-2 — the shared read happens once per gateway call, not per provider access.** A provider's health can change between `prepare` and the moment the chain reaches it. Deliberate: the alternative is a read per provider per attempt, and the failover walk already handles a provider that fails on the call itself.
- **LIM-D5.8-3 — D5.6's REPROBE register and every stream-link state remain process-local** (LIM-D5.6-4). Argued in ADR-048 rather than deferred, but it does mean a worker restart still loses every re-probe ladder and every readiness/probation/latency window, and the first sweep afterwards starts at the base delay.
- **LIM-D5.8-4 — no cross-worker notification.** A worker learns of a change by reading, not by being told. A push over the existing Redis pub/sub would shorten the window in LIM-D5.8-1 but must not become the authority, because a missed message is a silent drift; it is an optimisation *on top of* the authoritative read, and it is not built.
- **LIM-D5.8-5 — the atomic scripts require Redis 7 semantics** (`redis.call('TIME')` under effect-based replication). That is what `docker/redis` runs and what the compose stack pins, but a deployment pointed at Redis 6 with `lua-replicate-commands no` would see the claim script rejected — and would then fall back to per-worker cool-downs rather than fail, which is the documented Redis-unavailable path arriving through an unexpected door.
- **LIM-D5.8-6 — no live multi-worker validation.** See above.

---

### D5.9 — p95 Delivery Latency & Latency Inside `health()` ✅ (2026-08-29)

**ADR-049.** Closes LIM-D5.4-3. Nothing committed, nothing pushed.

**The audit's central finding is that D5.4's reason for refusing a p95 was arithmetic, and the conclusion stopped one step short of it.** ADR-044 said "p95 needs a sample larger than any warm-up worth waiting through" and left it there. Under the nearest-rank method the p95 of `N` samples is the `ceil(0.95 * N)`-th smallest, and that expression equals `N` for every `N` up to 19 — so a p95 over D5.4's nine-sample window **is the maximum**, which is exactly the one-outlier sensitivity `LATENCY_WINDOW_SAMPLES = 9` was chosen to avoid. D5.4 was right that nine cannot carry a p95; it never asked what can. `ceil(0.95 * N) < N` first holds at **`N = 20`**. That is where `LATENCY_TAIL_WINDOW_SAMPLES` comes from, and it makes the constant a derivation rather than a judgement — the difference between closing LIM-D5.4-3 and closing it with a number that has precision it does not have.

**The second finding is that ADR-044's other objection survives and shaped the design.** It objected to latency becoming *a counter* — accumulated on `ProviderHealth`, alongside evidence from calls a pushed feed never makes. It never objected to health *reporting* cadence. So the profile is **derived on every `health()` read and never stored**: not a counter, cannot drift from the intervals it summarises, and — the bug this avoids — cannot keep reporting a cadence after the feed goes stale or reconnects, because nothing has to remember to clear it.

**The third finding is that D5.9 needed no change to D5.8's shared state, for a structural reason rather than a careful one.** Only a provider that is *pushed into* has a delivery cadence, and exactly those providers declare `health_is_shared = False` (ADR-048). The set of providers with latency and the set whose health is shared are **disjoint by construction**, so no code path exists along which a cadence could reach Redis. Both halves are pinned by mutation.

**One series, two windows — and the D5.4 definition is untouched.** The deque widened from 9 to 20; the **median reads its newest 9** and the p95 reads all 20. `delivery_latency` is still the median of the last nine intervals, still establishes at nine, and is bit-identical to what a nine-long deque produced, because widening added older samples *behind* the ones the median reads. One recording site, one eviction rule, one reset, one monotonic clock. The two warm-ups are not ADR-044's drift hazard — that was one statistic with a `maxlen` and a separate warm-up free to disagree; here each statistic's window *is* its own warm-up, so a feed legitimately has a median and no tail between its 9th and 20th delivery, and `health()` is built to say so.

**Ranking is unchanged, and that is a refusal rather than an omission.** §7's `f(..., p95_latency)` is a continuous score ADR-044 rejected wholesale, so no surviving specification asks for p95 in selection — only a formula the platform already declined. Adding a fourth sort element would be new failover policy on evidence nobody has. Falsified in both directions: a mutation making p95 the selection metric and a mutation appending it as a fourth element are each caught.

**`health()` contract.** Gains `latency: {established, p50_seconds, p95_seconds, samples}`. Each statistic's `None` is its own "not established" — never `0` (which reads as instantaneous delivery, the inversion `LATENCY_RANK_UNKNOWN` exists to prevent) and never `math.inf` (a sort key that is not JSON). No name, broker vocabulary, credential or raw monotonic instant; `samples` is a count. `status()` gains nothing, `describe()` gains the p95 beside the p50 — the three contracts stay separate and are swept in all three directions.

**Files.** New: `backend/tests/test_provider_latency_p95.py`. Changed: `providers/base.py` (`LatencyProfile`, `ProviderHealth.latency`, `health()` refresh, the `latency_profile` default, `describe()` reads through `health()`), `providers/streaming.py` (`LATENCY_TAIL_WINDOW_SAMPLES`, `LATENCY_TAIL_PERCENTILE`, widened deque, `delivery_latency_p95`, `latency_profile`, the shared `_percentile_over` helper, `describe()`), `providers/__init__.py` (exports), `tests/test_provider_latency.py` (two tests, below). **No broker adapter, codec, transport, authentication, subscription or recovery path was touched; no `MarketTick` change; no source_manager, gateway, registry, health_state, API, route or frontend file was changed.**

**Two existing tests were changed, and they are reported rather than quiet.** `test_the_window_is_bounded_and_never_grows` pinned the deque's `maxlen` at `LATENCY_WINDOW_SAMPLES`; the property (fixed-size, one series) is unchanged and only the size it is pinned at moved. `test_old_latency_cannot_dominate_indefinitely` asserted the whole buffer was homogeneous after nine slow deliveries, using the raw deque as a proxy for the median's window; it now asserts the median's window directly — the property it was always about — and additionally asserts the tail evicts too, just later.

**Falsification: 28 mutations, 27 RED.** Covering p95 calculation, percentile method, sample eviction, insufficient-sample handling, reconnect reset, monotonic clock, `health()` exposure, unknown-latency representation, stale-feed and readiness interaction, ranking (three ways), the D5.8 shared/local boundary (two ways), broker isolation, consumer/status payload isolation and the one-series rule. **Three findings came out of the pass and all three changed the work:**
- *The rounding mode is inert at the published window.* `0.95 * 20` is exactly 19.0, so `floor` and `ceil` agree and a mutation between them changes nothing the shipped configuration can see. Not a gap in the code but a limit on what a test of the shipped constant can falsify, so the method is now pinned at a window where the two differ (nine, where `ceil(8.55) = 9` and `floor(8.55) = 8`).
- *`describe()` was not swept for raw monotonic instants*, only `health()`. A mutation adding `_last_evidence_at` to `describe()` passed. Now both surfaces are swept, against a clock started at a realistic process uptime so a leaked instant is unmistakable.
- *One mutation stays GREEN and is intentional defence in depth.* Making `apply_shared_health` explicitly blank the profile changes nothing observable, because the profile is derived on every read — there is no durable field to corrupt. The recomputation is the real guarantee, and the mutation that removes *it* is RED. Recorded in the test rather than counted as a pass.

**Regression.** Baseline (no Redis) **3790 passed / 15 failed / 41 skipped / 4 xfailed**; final (Redis up, so the D5.8 suite runs instead of skipping) **3862 passed / 15 failed / 0 skipped / 4 xfailed**. It reconciles exactly: 3790 + 41 previously-skipped + **31 new tests** = 3862. The 15 failures are the known pre-existing Docker set in `test_entrypoint_log_level.py`, unchanged in count and identity. The D4/D5 suites (949 tests) pass under **five shuffled file orderings**.

**Quality.** Blocking flake8 clean on all changed/new files; advisory flake8 (`--max-line-length=100`) reports one pre-existing E501 on `bind_sink`, untouched by this sprint. isort and black clean on the new file. `black --check` and `isort --check` were confirmed to *already* disagree with `base.py`, `streaming.py`, `providers/__init__.py` and `test_provider_latency.py` **before** any D5.9 edit, so those files were deliberately not reformatted.

**Security.** Real logging stack at DEBUG with live-looking fake credentials in the shapes the five adapters use (Kite access token, Upstox JWT, Angel feed token, Fyers app id, Dhan client id, API secret, password). Verified: no credential in the latency samples, in `health()`, in `describe()` or in the DEBUG stream; no broker vocabulary in the cadence block; no raw monotonic instant on either introspection surface; no cadence vocabulary and no credential in any live Redis key or value (swept against a real Redis 7, with both writes asserted `ok` so the sweep is not vacuous); no cross-user visibility. The owner id in a D5.8 Redis *key* is the intentional per-user scoping, not a leak, and is absent from every value.

**Deliberately NOT done in D5.9** — no exchange-timestamp latency (LIM-D5.4-1 stands), no p95 in ranking (argued, not omitted), no instrument sharding, no chaos suite, no frontend explanation (LIM-D5.5-2 stands), no Groww, no INDmoney, no new broker adapter, no D5.10; no change to `MarketTick`, any broker adapter or codec, broker authentication/transport/subscription/recovery, the D5.8 ownership boundary, readiness/probation/stale-feed/entitlement semantics, the REST quote path or the frontend; nothing committed and nothing pushed.

**LIVE VALIDATION: NOT PERFORMED.** No broker session was available. Every tick in this sprint is synthetic and every clock is injected; the Redis used is a local container. Outstanding smoke test, carried forward: with one real broker feed attached, confirm that (a) a p95 establishes after 20 accepted batches and is >= the p50, (b) a reconnect clears both, (c) `health()` on the admin surface shows the cadence and no broker identity, and (d) the p95 over a real session is plausible against the feed's observed cadence.

**Limitations carried out of D5.9.**
- **LIM-D5.9-1 — the p95 excludes exactly one sample.** At `N = 20` the rank is 19, so the statistic tolerates one catastrophic gap and no more. That is the *most* tolerance the smallest non-degenerate window can give; a feed with two bad deliveries in twenty reports the second-worst, which is correct but means the figure moves sooner than a p95 over a larger sample would. Widening further is one constant to change if operational evidence asks for it.
- **LIM-D5.9-2 — the rounding mode is unobservable at the shipped window** (see the falsification finding above). Pinned at a second window so the general method stays falsifiable, but no test of the shipped configuration alone can distinguish `floor` from `ceil`.
- **LIM-D5.9-3 — the p95 is per-worker and per-socket, like every other live-link statistic** (LIM-D5.8-3). Two workers holding two sockets for two users report two cadences and neither is a deployment-wide figure. Correct by ADR-048's argument, but it means an operator reading one worker is reading one link.
- **LIM-D5.9-4 — the tail warm-up is longer than the median's.** A feed has no p95 for its first 20 deliveries. Negligible on a real feed and irrelevant on a slow one (which is stale, and reports no cadence at all), but it means the figure is absent exactly when a feed has just reconnected — which is when an operator is most likely to look.
- **LIM-D5.9-5 — no live validation.** See above.


---

## D5.10 — Instrument Sharding & Multi-Connection Streaming

Status

COMPLETED (2026-08-30, ADR-050)

Priority

High

**What it is.** One logical `StreamingTickProvider` may now own several broker connections, so a subscription larger than one connection's limit is split across as many as it needs instead of being trimmed to a deterministic prefix. Consumers see exactly one canonical provider and one coherent stream of `MarketTick`s. **A shard is not a provider.**

**Architecture audit (17 questions, answered from the code).** The subscription set is built in `BrokerEngine.start_stream` from `broker_gateway.stream_instruments`; each broker's limit lived as a private module constant that *truncated*; four brokers cap per connection and one caps per **session**, which sharding cannot raise; subscriptions are sent by `BrokerStreamChannel.subscribe_frames` through the one generic transport; `BrokerStreamManager` keyed on `(user, broker, channel)` since D4.7, so a second connection of one channel would have silently replaced the first; readiness, D5.2 probation, D5.3 freshness and D5.4/D5.9 latency were all **per-provider scalars** describing one socket; D5.6's REPROBE is keyed on the channel; D5.7/D5.8 already scope live-socket health as process-local (`health_is_shared = False`); D5.1's flap suppression is already per `BrokerStream`, so it is per connection for free; Fyers' `open(session, credentials)` is per connection and therefore per shard with no change.

**Was a generic contract change required? Yes — two, both broker-neutral, both defaulting to the pre-sprint behaviour.**
1. `BrokerStreamChannel.max_instruments_per_connection` / `max_connections` (and their adapter-level equivalents). Nothing existing could express "how many instruments does *one connection* hold", and the existing constants could not be reused because they mix three different kinds of limit.
2. A shard dimension on the transport registry key and an optional `shard` on the provider's push/link surfaces. `(user, broker, channel)` cannot address two connections of one channel; per-provider scalars cannot describe connections that fail independently.

Neither widens the channel's *methods*: `endpoint`, `open`, `subscribe_frames`, `connect_error` and `decode` are untouched, so an unmigrated broker fails at import rather than on a live socket — the trade D4.7 and D4.10 both refused.

**Per-broker limits, declared not copied.** Zerodha 3,000/connection; Upstox 5,000 keys/connection (on the *market* channel); Fyers 5,000/connection; Dhan 5,000/connection capped at **5 concurrent connections** (Dhan disconnects the *oldest* past its ceiling); **Angel One declares none** — its 1,000 is a per-session quota and is still trimmed with a warning. Per-frame batching (Dhan 100, Fyers 1,500) is wire framing and is deliberately not a shard limit.

**Semantics.** Readiness and per-symbol coverage stay "at least one connection is delivering", so a lost connection does not blank the instruments its siblings are still serving. Every provider-level claim becomes "**every** declared connection": `_last_evidence_at` is the minimum over connections and `_ready_since` the maximum, so `has_fresh_evidence` and `stability` needed no change and a healthy connection cannot mask a dead one. Latency is measured **per connection** and aggregated by the **worst**, so shard count is never a latency advantage. Resharding is make-before-break: unchanged connections are left alone, and connections leaving the plan are stopped last.

**Files changed.** New: `backend/services/brokers/sharding.py`, `backend/tests/test_instrument_sharding.py`. Modified: `brokers/streaming.py`, `brokers/base.py`, `brokers/stream.py`, `brokers/market_feed.py`, `broker_engine.py`, `market_engine/providers/streaming.py`, `market_engine/providers/__init__.py`, the five stream adapters (declarations only), and four existing test modules whose assertions read the internals the ledger moved.

**Tests.** 80 new in `test_instrument_sharding.py`, covering planning, per-broker limits, the shard-is-not-a-provider invariant, readiness/freshness/probation/latency aggregation, failure isolation (first/middle/last/multiple/all), reconnect, resharding, the isolation matrix including a fictional broker through the real `attach_market_feed` seam, instrument identity for all five brokers, and the security sweep.

**Falsification.** 36 mutations applied to the shipped source and reverted; **36/36 observed RED**. Five failed on the first pass and were reformed rather than counted: one malformed anchor, one test that made two connections identical so a min/max mutation could not be seen, and three real coverage gaps — nothing drove a tick or a link change through the engine's bound callbacks and checked which connection it landed on, which is now `test_a_tick_is_attributed_to_the_connection_it_arrived_on`.

**One defect was found by self-review rather than by a test, and is recorded because that is the interesting half.** `shard=None` legitimately means two different things — *every* connection to `_discard_evidence` (what `disconnect()` needs) and *this feed's only* connection to `mark_link_up` / `mark_link_down` / `on_raw` (what every pre-D5.10 caller means). The first draft let the unresolved `None` fall from the second meaning into the first, so `mark_link_down()` with no shard named on a three-connection feed blanked all three connections' prices while marking exactly one of them down — a feed left with no coverage and two sockets it still believed were up. No test caught it because production never passes `None` there (`set_market_feed_link` defaults to the string `"0"`), and a single-connection feed cannot distinguish the two meanings at all. The shard is now resolved once at the top of each method; `test_an_unnamed_connection_means_the_default_one_not_all_of_them` is the pin, and M36 is its mutation.

**Regression.** Baseline **3821 passed / 15 failed / 41 skipped / 95 deselected / 4 xfailed**; final **3901 passed / 15 failed / 41 skipped / 95 deselected / 4 xfailed**. It reconciles exactly: 3821 + **80 new tests** = 3901. The 15 failures are the known pre-existing Docker set in `test_entrypoint_log_level.py`, unchanged in count and identity, and **no other test failed in any run**. Confirmed under a sequential ordering and **two shuffled file orderings**, all three reporting the identical counts.

One unrelated flake was observed once and did not reproduce: `test_backup_restore.py::TestVerification::test_wrong_passphrase_is_detected` failed in a single mid-sprint full-suite run and passed in isolation and in every subsequent full run. It is a GPG/timing flake in a module D5.10 does not touch, and is recorded here rather than absorbed into this sprint's baseline.

**Quality.** Blocking flake8 clean on every changed and new file; the two remaining findings in `broker_engine.py` (one F401, one E501) were confirmed present at HEAD before any D5.10 edit. The advisory gates were run and deliberately not applied: `black --check` and `isort --check` were confirmed to *already* disagree with `stream.py`, `providers/streaming.py`, `test_provider_latency.py`, `test_provider_probation.py` and `test_broker_streaming.py` **before** any D5.10 edit, so the new files match their neighbours' style rather than the formatter's, and no unrelated file was reformatted.

**Security.** Full sharded lifecycle driven at DEBUG with live-looking fake credentials. Verified: no credential in a shard id, a registry key, a task name, `describe()`, `health()` or any log line; no shard vocabulary in a quote payload, in `source_manager.status()`, or in `describe()` (which reports a *count*, `connections`, not identifiers); shard ids are positional integers-as-strings carrying no symbol, user or broker.

**Deliberately NOT done in D5.10** — no D5.11 chaos testing, no exchange-timestamp latency, no p95 changes, no new brokers (no Groww, no INDmoney), no order or trading APIs, no REST quote changes, no frontend changes, no unrelated health redesign, no change to `MarketTick`, any codec, instrument identity, the D5.8 ownership boundary or the recovery register's key; nothing committed and nothing pushed.

**LIVE VALIDATION: NOT PERFORMED.** No interactive broker session exists, and no account in this repository holds a portfolio exceeding any broker's per-connection limit. A concrete eight-step smoke-test checklist is in BROKER_INTEGRATION.md.

**Limitations carried out of D5.10.**
- **LIM-D5.10-1 — no concurrent-connection ceiling is declared for four of the five brokers**, because the repository documents one only for Dhan and D5.10 does not invent numbers. A portfolio far beyond one connection is therefore planned into as many connections as it needs, and a broker that refuses the excess refuses it at the handshake, where D5.1's ladder paces the retry.
- **LIM-D5.10-2 — Angel One's 1,000-token quota is still enforced by trimming.** It is a per-session quota, not a per-connection ceiling, and sharding cannot raise it. The correct outcome, not an unfinished one.
- **LIM-D5.10-3 — a partially failed sharded feed is ranked below the delayed baseline for every instrument**, including the ones its healthy connections are still delivering, until the lost connection is restored and has served a full probation window. This follows mechanically from ADR-043 (stability reads freshness) and ADR-042 (probation ranks below steady). The data is preserved — the feed stays eligible, keeps its coverage, and answers whenever nothing steadier remains — but it is not preferred. A per-shard stability term would be a second ranking system and would let a permanently dead connection hold the primary position.
- **LIM-D5.10-4 — the reconnect reset is now one control rather than two.** ADR-042's second control stamped a provider-level timestamp on a readiness transition, which a sharded feed cannot have; it moved to the connection that earns it. Recorded rather than tidied away.
- **LIM-D5.10-5 — resharding reuse compares the session and credential dicts by equality.** A session that changes in a way equality cannot see would leave a connection running on stale material; the failure direction of anything it cannot prove unchanged is a rebuild, which is the pre-D5.10 behaviour.
...
- **LIM-D5.12-1 — the feed reports `available` while a trial is only offered. NEW, and the sprint's only finding.** ✅ **CLOSED by D5.13 (ADR-053).** During a sustained total outage of the only provider serving a capability, the consumer-facing feed state flips to `available` at each cool-down expiry — before anything has answered — and back to `unavailable` once the trial is spent and fails. Nothing is fabricated: `status()` resolves through the same path a request does, and a re-admitted provider genuinely is a candidate. It is bounded by the ladder and cannot become a flap. Making `status()` ignore probes would remove the blink at the price of a consumer surface that disagrees with resolution, so it is pinned as it stands and left to the sprint that owns the consumer contract.
- **LIM-D5.5-2 — the user is told their tier moved, not why.** ✅ **CLOSED on the backend by D5.13 (ADR-053)**; its frontend half re-recorded as LIM-D5.13-1 rather than silently absorbed. Paired with LIM-D5.12-1 exactly as predicted: one surface, one root cause.
...
## D5.13 — Consumer Feed Contract

Status

COMPLETED (2026-08-31, ADR-053) — **production change in four files; live validation not performed**

Priority

High

**What it is.** The sprint that owns `SourceManager.status()` — the one payload the frontend, the AI context and `/market/status` are allowed to see — and the one that had to decide LIM-D5.12-1 and LIM-D5.5-2 together. The audit found them to be **one defect, not two**, and the brief's instruction to prove what was wrong before implementing is what produced that finding.

**Architecture audit — the root cause.** `Resolution.as_status()` answered the **resolution** question — *"is there a provider I will try?"* — on a surface whose consumers ask the **delivery** question — *"is my feed serving me usable data?"*. For every provider that has passed health, readiness, freshness and coverage those two have the same answer, which is why the collapse survived from D1 to D5.12 unnoticed. They come apart in exactly one place, and D5.7 created it: a provider re-admitted at `DOWN` for one trial is a real candidate and a real non-answer, so the two-value contract had to call it one of the two — and called it `available`, with a `delayed` freshness tier, about a provider whose last eight calls had all failed.

Reproduced end to end through the real path (`healthy → DOWN → cooldown → trial → failure → cooldown`), the consumer saw:

    healthy       {"state":"available",   "tier":"delayed", "reason":null,                 "capabilities":["quotes"]}
    DOWN          {"state":"unavailable", "tier":null,      "reason":"all_providers_down", "capabilities":[]}
    cooldown out  {"state":"available",   "tier":"delayed", "reason":null,                 "capabilities":["quotes"]}   <- health still DOWN, 0 calls made
    trial failed  {"state":"unavailable", "tier":null,      "reason":"all_providers_down", "capabilities":[]}

The same collapse explains LIM-D5.5-2 from the other side. `UnavailableReason` is a property of the *current* resolution and can always be re-derived; the cause of a **transition** cannot, because `detach_market_feed` **unregisters the provider** — one line later nothing in the Source Manager knows the feed existed. The reason was not missing by oversight; there was no place in the model for it to be derived from.

**The seven-way semantic distinction the brief asked for**, and where each is answered: *exists* (a constructed provider object — invisible to resolution); *registered* (`ProviderRegistry`); *eligible* (`is_eligible_for` — entitlement, link, readiness, per-symbol coverage); *selected* (`chain[0]`, recomputed per request, never stored — D4.5); *being probed* (D5.7's cool-down expired and `due_from` offered it); *successfully recovered* (`record_success` moved health off `DOWN` — the only evidence the platform accepts); *delivering usable data* (**this had no name before D5.13**, and `available` was silently standing in for it).

**What changed.** `state` is now three-valued — `available` | `recovering` | `unavailable`. `recovering` is **derived, never stored**: true exactly when the selected provider's health is `DOWN`. It is a refinement of *not available*, so a consumer branching `state == "available"` takes its degraded branch — the safe direction. It carries `tier: null` and no capabilities, because a tier is a claim about data. **Only `DOWN` qualifies**: `UNKNOWN` is unproven-not-failing (the baseline is `UNKNOWN` at every boot) and `DEGRADED` is still serving; both are pinned. **Resolution is untouched** — the probe is still offered, ranked last and spent by the request that reaches it, because a status surface that suppressed the probe would suppress the recovery and restore the ADR-029 deadlock.

`FeedChangeReason` — `entitlement_refused` | `session_expired` | `feed_disconnected` — rides the `provider.status` **event** and never `status()`, whose four keys are unchanged. One value per platform path that unregisters a live feed; the vocabulary describes the *feed*, never the broker, and `publish_status` coerces through the enum so an off-vocabulary string is refused rather than published.

**Was a production change necessary? Yes**, and the audit proved it rather than assuming it. The `recovering` distinction cannot be projected in the gateway (which has no information the Source Manager lacks) and must not be projected in `resolve_feed` (which would break recovery). The transition reason cannot be derived anywhere, at any layer, because its subject has been unregistered — it had to be *carried* from the one caller that still knows.

**A defect found while wiring it, which would have made the sprint a silent no-op.** `unregister_streaming_provider` disconnected the provider **before** publishing. The disconnect drives readiness backwards, firing `_on_provider_readiness`, which published the user-scoped demotion *from inside the teardown and unexplained*; change gating then dropped the real, explained publish as a repeat. Every test that only asserted "the owner was told" would have stayed green. Fixed by unbinding the readiness listener before the disconnect — right on its own terms, since a readiness change produced by tearing a provider down is an artifact of the teardown, not news about a user's feed. Pinned by `test_the_explained_event_is_not_beaten_to_the_bus_by_the_teardown`, which asserts **exactly one** user-scoped event carrying both `previous_tier: "streaming"` and the reason.

**A documented claim found to be false.** DECISIONS.md justified deferring LIM-D5.5-2 partly because "the frontend already renders the tier flip from it". **It does not, and never has.** `provider.status` has domain `provider`, unmapped by `DOMAIN_CHANNEL`, so the platform-scoped event broadcasts to a channel `RealtimeProvider.jsx` never subscribes to; the user-scoped event *is* delivered to the owner's sockets and `realtimeStore.applyEvent` drops it at `default: break`. The frontend learns the tier only from `source_tier` on REST payloads. Corrected in place rather than quietly fixed.

**Files changed.** Production: `services/market_engine/source_manager.py`, `services/market_engine/gateway.py`, `services/brokers/market_feed.py`, `services/broker_engine.py`. Tests: `backend/tests/test_consumer_feed_contract.py` (new), `backend/tests/test_provider_health_recovery.py` (3 tests updated). Docs: `.claude/DECISIONS.md` (ADR-053), `.claude/MARKET_DATA_ARCHITECTURE.md`, `.claude/BROKER_INTEGRATION.md`, `.claude/TASK.md`. **No frontend file was touched, no adapter, codec, transport or `MarketTick` field, and no `active_tier()` / `source_tier()` semantics.**

**Tests.** 24 added in a new suite, written **before** the implementation. Section A pins the outage contract (state, tier, capabilities, the recovery back to `available`, the probe still being offered and spent, a probe behind a healthy provider, `UNKNOWN` at startup, `DEGRADED`, payload shape). Section B pins the explanation (the reason, the single un-beaten event, the steady state staying clean, broker-neutrality, the closed vocabulary, owner-scoping, the platform broadcast staying silent, the unexplained payload unchanged). Section C pins the D5.2–D5.12 invariants across the change (Yahoo untouched, probation still ranking and not filtering, the four-way unavailable diagnosis intact, diagnostics still holding the detail, DEBUG logging with live-looking credentials, the no-broker-import sweep).

**Falsification: 19 mutations, 19 RED** — remove the guard; make probe eligibility equal successful recovery; widen it to `UNKNOWN`; widen it to `DEGRADED`; key it off the presence of a probe rather than the selection; keep claiming a tier; advertise probe-only capabilities; suppress the probe in resolution; drop the explanation; leak it into the steady state; make it broker-specific; collapse the vocabulary; broadcast one user's explanation; restore the teardown ordering; send the explanation to another user; leak provider identity; report `recovering` as plain `unavailable`; make `recovering` permanent. **Two were reported as malformed and reformed rather than counted:** one used an `or` fallback that never fired on any path under test, and one removed the enum coercion while every call site still passed an enum member — byte-identical output, therefore inert. The second exposed a real gap and produced a 24th test (`test_the_transition_vocabulary_is_closed_against_a_broker_string`), after which it turned RED.

**Regression.** Baseline recorded at `d97b89f` **before editing**: 15 failed / 4095 passed / 49 skipped / 95 deselected / 4 xfailed. Final: **15 failed / 4119 passed / 49 skipped / 95 deselected / 4 xfailed**. Reconciles exactly: 4095 + 24. **The 15 `test_entrypoint_log_level.py` failures are the pre-existing Docker-dependent set carried since D3 — preserved and unabsorbed, identical in both runs** (no Docker daemon was reachable in this sprint's environment). `flake8 --select=E9,F63,F7,F82,F811,F632` (the blocking gate) clean repo-wide; the advisory pass on the changed files reports **only two findings that are present at HEAD in their pristine state** and were not introduced here — deliberately not reformatted. **`pytest-randomly` is not installed in this venv**, so shuffled ordering was approximated rather than claimed: the fourteen D4/D5/chaos suites run **1179 passed / 49 skipped** in forward, reversed and new-file-last orderings, and every file also passes in isolation — the permutation that actually exercises the shared `provider_registry` and `event_bus`.

**Security.** The real logging stack exercised at **DEBUG** through the real entitlement-refusal path with a live-looking JWT planted in the broker's stream endpoint. 36 log lines captured, **0 containing the credential**. The consumer payload was checked against twelve forbidden classes — credential, `token=`, broker name, wire code `806`, `not_subscribed`, `wss://`, provider identity `feed:u1`, the audit topic, transport error text, recovery-class vocabulary, health vocabulary, shard identity — **none present**. The transition log line reads `state=available tier=delayed reason=None change=entitlement_refused` and names no broker.

**Deliberately NOT done in D5.13** — no frontend work (identified explicitly and left as LIM-D5.13-1, not assumed away), no change to `resolve_feed`'s ranking or filtering, no change to `active_tier()` / `source_tier()`, no new recovery mechanism and no second ladder, no unification of D5.6 REPROBE with D5.7 health recovery, no change to `MarketTick`, any adapter, codec or transport, no exchange-timestamp latency (LIM-D5.4-1 stands blocked; no defensible clock offset exists and none was invented), no instrument sharding, no chaos tests, no new brokers, no trading APIs, no D5.14; nothing committed and nothing pushed.

**LIVE VALIDATION: NOT PERFORMED.** No interactive broker session exists. Every test is hermetic — no socket, no broker API, no sleep on a cool-down — and this sprint claims nothing about how a real broker behaves.

**Limitations after D5.13.**
- **LIM-D5.12-1 — the feed reports `available` while a trial is only offered.** ✅ **CLOSED.** `recovering` is a third state that agrees with resolution and claims nothing about data; the probe is still offered and still spent.
- **LIM-D5.5-2 — the user is told their tier moved, not why.** ✅ **CLOSED on the backend.** The user-scoped `provider.status` now carries a broker-neutral `change_reason`. Its frontend half is re-recorded below rather than absorbed.
- **LIM-D5.13-1 — nothing in the frontend consumes `provider.status`. NEW.** The explanation now exists on the payload and no UI reads it: `realtimeStore.applyEvent` drops the `provider` domain at `default: break`, and the platform-scoped event goes to an unsubscribed channel. Closing it is a UI surface (which banner, which copy, which dismissal) and a product decision, not a contract fix — which is why D5.13 identified it rather than building it. **Note this is a correction, not a discovery of new breakage:** the surface was never wired, and the claim that it was is corrected in DECISIONS.md and ADR-053.
- **LIM-D5.13-2 — `recovering` is reasoned about with one polled provider. NEW, and inherited from ADR-052's review condition.** Every argument for the state's shape rests on the polled baseline being the only provider of its kind. A second polled provider makes "which state does the feed report when one is probing and one is serving?" a question again — it is answered correctly today (`recovering` follows the *selected* provider, pinned by `test_a_probe_behind_a_healthy_provider_does_not_make_the_feed_recovering`), but it becomes a live path rather than a hypothetical.
- **LIM-D5.6-2 / -3 / -4 / LIM-D5.8-3 / LIM-D5.6-5** — unchanged, and restated rather than re-litigated: `CONFIGURATION` classified and never recorded; a refused re-probe costs one connection; the REPROBE register is process-local **by argument** (ADR-048); no live validation.
